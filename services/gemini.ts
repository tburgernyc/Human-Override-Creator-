...
export const handleDirectorChat = async (message: string, currentProject: ProjectState, chatHistory: {role: 'user'|'model', content: string}[]): Promise<GenerateContentResponse> => {
  ...
  const response = await ai.models.generateContent({
    ...
    contents: [
      ...
      { role: 'user', parts: [{ text: `PROJECT_CONTEXT: ${JSON.stringify({ 
...
export const analyzeScript = async (script: string, seed?: number): Promise<{ 
  ...
  const data = JSON.parse(cleanJsonResponse(response.text || "{}"));
  return { 
    characters: data.characters.map((c: any, i: number) => ({ ...c, id: `char_${i}_${Date.now()}`, voiceId: VOICE_PRESETS[0].id, voiceSettings: { pitch: 0, speed: 1 } })), 
    scenes: data.scenes.map((s: any, i: number) => ({ ...s, id: i + Date.now(), cameraMotion: 'random_cinematic', transition: 'fade' })),
...
export const generateSceneVideo = async (imageBase64: string, prompt: string, aspectRatio: AspectRatio, resolution: Resolution, style: string = "Cinematic"): Promise<string> => {
    ...
    const blob = await (await fetch(`${operation.response?.generatedVideos?.[0]?.video?.uri}&key=${process.env.API_KEY}`)).blob();
