...
interface SceneCardProps {
    ...
    onRetryVideo: (sceneId: number) => void;
    onRetryAudio: (sceneId: number) => void;
...
export const SceneCard: React.FC<SceneCardProps> = ({
    ...
    const handleOptimizePrompt = async () => {
        ...
        try {
            ...
            console.error('[SceneCard] Visual prompt optimization failed:', e);
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" className="w-full nm-inset-input rounded-xl p-3 text-xs text-starlight outline-none border-none" value={editedDuration} onChange={e => { const v = parseFloat(e.target.value); setEditedDuration(isNaN(v) ? 1 : Math.max(1, v)); }} />
                        </div>
                        <div>
        ...
        <div className="nm-card overflow-hidden flex flex-col h-full group relative border border-white/5 bg-eclipse-black">
            ...
            <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="flex-1 space-y-2 mb-6">
                    ...
                    {asset?.error && (
                        <div className="flex items-start gap-2 text-[9px] text-solar-amber border-l-2 border-solar-amber/50 pl-2 py-1 bg-solar-amber/5 rounded">
                            <i className="fa-solid fa-triangle-exclamation text-[8px] mt-0.5"></i>
                            <span className="flex-1 line-clamp-2 font-medium">{asset.error}</span>
                        </div>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/5">
                    ...
                    <div className="flex gap-2">
                        ...
                        <div className="relative group/actions">
                            <button className={`h-9 px-5 rounded-xl ${asset?.status === 'error' ? 'nm-button border border-solar-amber/30 text-solar-amber' : 'nm-button-gold text-white shadow-nm-gold'} flex items-center justify-center text-[9px] font-black uppercase tracking-[0.2em] transition-all hover:scale-105 active:scale-95`}>
                                {asset?.status === 'error' ? 'Retry' : 'Take'}
                            </button>
                            <div className="absolute bottom-full right-0 mb-3 w-56 nm-panel rounded-2xl shadow-2xl hidden group-hover/actions:block z-40 py-2 border border-white/5 animate-in fade-in slide-in-from-bottom-2">
                                ...
                                {/* Granular retry buttons - shown when there are errors */}
                                {(asset?.status === 'error' || asset?.error) && (
                                    <>
                                        <div className="h-px nm-inset-input mx-2 my-1"></div>
                                        <div className="px-3 py-2">
                                            <p className="text-[8px] text-solar-amber uppercase font-black tracking-widest mb-2">Granular Retry</p>
                                        </div>

                                        {/* Retry Image - always available for errors */}
                                        <button
                                            onClick={() => onRetryImage(scene.id)}
                                            className="w-full text-left px-4 py-3 text-[10px] font-bold text-solar-amber hover:bg-solar-amber hover:text-white transition-colors uppercase"
                                        >
                                            <i className="fa-solid fa-image mr-3"></i> Retry Image
                                        </button>

                                        {/* Retry Video - only if image exists */}
                                        {asset?.imageUrl && (
                                            <button
                                                onClick={() => onRetryVideo(scene.id)}
                                                className="w-full text-left px-4 py-3 text-[10px] font-bold text-solar-amber hover:bg-solar-amber hover:text-white transition-colors uppercase"
                                            >
                                                <i className="fa-solid fa-film mr-3"></i> Retry Video
                                            </button>
                                        )}

                                        {/* Retry Audio - always available for errors */}
                                        <button
                                            onClick={() => onRetryAudio(scene.id)}
                                            className="w-full text-left px-4 py-3 text-[10px] font-bold text-solar-amber hover:bg-solar-amber hover:text-white transition-colors uppercase"
                                        >
                                            <i className="fa-solid fa-microphone mr-3"></i> Retry Audio
                                        </button>
                                    </>
                                )}
