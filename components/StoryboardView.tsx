...
// B3 (audit slice): pure body content for the unified AssetWorkspaceModal.
// `onSelectClose` lets the caller close the workspace when a thumbnail is
// chosen — the legacy default modal still wires this to its own onClose.
interface StoryboardViewBodyProps {
  scenes: Scene[];
  assets: GeneratedAssets;
  onSelectScene: (id: string) => void;
  onSelectClose?: () => void;
...
export const StoryboardViewBody: React.FC<StoryboardViewBodyProps> = ({ scenes, assets, onSelectScene, onSelectClose }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
    {scenes.map((scene, idx) => {
      ...
      return (
        <div
          key={scene.id}
          onClick={() => { onSelectScene(scene.id); onSelectClose?.(); }}
    ...
    <div className="fixed inset-0 z-[400] bg-eclipse-black/98 backdrop-blur-3xl flex items-center justify-center p-6 animate-in fade-in duration-500">
      <div className="w-full max-w-7xl h-full flex flex-col bg-eclipse-black border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-12 scrollbar-hide">
          <StoryboardViewBody scenes={scenes} assets={assets} onSelectScene={onSelectScene} onSelectClose={onClose} />
        </div>
